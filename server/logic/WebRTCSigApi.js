 ...
 */
function genPrivMapEncrypt(userid, roomid, expire) {
  //视频校验位需要用到的字段
  /*
  ...
  */
  ...
  bytes[offset++] = (config.im.sdkAppID & 0xFF000000) >> 24;
  bytes[offset++] = (config.im.sdkAppID & 0x00FF0000) >> 16;
  bytes[offset++] = (config.im.sdkAppID & 0x0000FF00) >> 8;
  bytes[offset++] = config.im.sdkAppID & 0x000000FF;
  ...
  let signature = sign(content, private_key);
  json['TLS.sig'] = signature.toString("base64");
  ...
  return gzcompress(json_text);
}

module.exports = {

  // genPrivMapEncrypt: function (userid, roomid , cb) {
  //   //生成privMapEncrypt
  //   let privMapEncrypt = genPrivMapEncrypt(userid, roomid);
